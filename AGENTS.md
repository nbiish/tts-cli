...
- **Default model is `pocket-tts`** (Kyutai PocketTTS, the fast default — zero-shot voice cloning, cross-platform CPU + Apple Silicon MPS/CUDA; ~3.7x faster cold than IndexTTS-2.5 GGUF). It is **system-agnostic** — runs on macOS (CPU/MPS), Linux/Windows/WSL (CPU/CUDA), no accelerator required (override device with `POCKETTTS_DEVICE=cpu|mps|cuda`). Use `--lang EN|ES|IT|DE|PT|FR` when the summary is non-English (PocketTTS ships European-language models; unmapped codes fall back to english). For voice cloning, pass `--voice <ref.wav>`; a cross-platform fallback reference ships at `examples/default_voice.wav` (used automatically if no `--voice` is given). Use `--quality` to switch to the full-precision Python IndexTTS-2.5 (MPS/CUDA) — slower but highest-fidelity multilingual (ZH/EN/JA/ES/AR) cloning. Use `--model index-tts` for the IndexTTS-2.5 GGUF (audio.cpp) path.
