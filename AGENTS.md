...
- **Default model is `index-tts`** (IndexTTS-2.5, the sole engine). It is **system-agnostic** — runs on macOS (Metal), Linux/Windows/WSL (CUDA/Vulkan), and CPU-only via audio.cpp; the backend auto-detects (override with `AUDIOCPP_BACKEND=cpu`). Use `--lang ZH|EN|JA|ES|AR` when the summary is non-English. For voice cloning, pass `--voice <ref.wav>`; a cross-platform fallback reference ships at `examples/default_voice.wav` (use `--voice examples/default_voice.wav` if no operator-recorded reference is set).
