...
- **Model selection (operator-chosen; `auto` resolves to the configured default):** the operator picks the default once with `cli-tts --set-default {model}` (persists to `~/.tts-cli/default_model`; choices: `pocket-tts` | `kitten-tts-nano` | `kitten-tts-mini` | `index-tts` | `index-tts-quality`). `cli-tts --list` shows status. Then agents just call `cli-tts --prompt "..."` and the chosen default runs with its own native prompting — no per-call model prompting. Override env: `TTS_CLI_DEFAULT_MODEL`. Engines:
  - `pocket-tts` (built-in fallback default): Kyutai PocketTTS, zero-shot voice cloning, cross-platform CPU + Apple Silicon MPS/CUDA; ~3.7x faster cold than IndexTTS-2.5 GGUF. System-agnostic (macOS CPU/MPS, Linux/Windows/WSL CPU/CUDA; `POCKETTTS_DEVICE=cpu|mps|cuda`). `--lang EN|ES|IT|DE|PT|FR` (unmapped → english). `--voice <ref.wav>` clones; bundled fallback `examples/default_voice.wav` used if no `--voice`.
  - `kitten-tts-nano` / `kitten-tts-mini`: ultra-lightweight CPU ONNX, fixed built-in voices (no cloning), smallest footprint / fastest cold load. `--voice` selects a built-in voice name (e.g. `expr-voice-5-m`).
  - `index-tts`: IndexTTS-2.5 Q8 GGUF via audio.cpp (Metal/CUDA/Vulkan/CPU; `AUDIOCPP_BACKEND=cpu`).
  - `index-tts-quality` (`--quality`): full-precision Python IndexTTS-2.5 (MPS/CUDA), highest-fidelity multilingual (ZH/EN/JA/ES/AR) cloning. Slowest.
- If the `local-tts-mcp` MCP server is enabled in the harness, prefer it over the CLI.
- **Durable transcript (mandatory):** the suggestion portion of every `cli-tts` call (the "Next step: ..." hardened-engineer recommendation only — NOT the concise summary, to keep the file token-economical for cross-agent context) is appended to `AGENTS-TTS-COMMS.txt` at the repo root — ISO-8601 timestamped, with model/lang/voice. This is automatic (the CLI writes it on success); do not suppress it. Calls with no "Next step:" segment write nothing. Track the file in git alongside `AGENTS.md`.
