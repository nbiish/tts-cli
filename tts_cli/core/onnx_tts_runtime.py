...
DEFAULT_OUTPUT_DIR = Path.home() / ".tts-cli" / "generated_audio"
try:
...
class OnnxTtsRuntime(OrtCpuRuntime):
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        ...
        import scipy.signal
        import math
        data, sample_rate = sf.read(str(Path(reference_audio_path).expanduser().resolve()), dtype="float32")
        if data.ndim == 1:
            data = data[None, :]
        else:
            data = data.T
